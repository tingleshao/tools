...
{
   BaseSocketData socketData;
   socketData.data.allocate(dataSize);
...
/*
   ...
   for( int i = 0; i < recvCount; i++ ) 
   {
      ...
      if( running ) {
         ...
         clientIndex = baseSocket.createTcpClient("localhost", port );
         if( clientIndex < 0 ) {
           printf("BaseSocket_test: Unable to createTcpClient: %d!\n", clientIndex );
           running = false;
           rc = false;
         }
   ...
   while(sendTime.elapsed() < 2.0 )
   {
      for( int i = 0; i < recvCount; i++ ) 
      {
         ...
         if( running ) {
            ...
            if( baseSocket.sendData( clientIndex,  (uint8_t *)data,  strlen(data)) < 0 ) {
...
//   printf("Sent %d message in %lf seconds = %lf per second\n", messageCount, elapsed, (double)messageCount/elapsed);
   running = false;
